...
/**
 * Created by Nikita on 12/4/2017.
 */

public class StlModel1  extends ArrayModel1 {
    private static int HEADER_SIZE = 80;

    public StlModel1(@NonNull InputStream inputStream) throws IOException {
        super();
    ...
    @Override
    public void initModelMatrix1(float boundSize) {
        final float zRotation = 270f;
        final float xRotation = -90.0f;
        initModelMatrix1(boundSize, xRotation, 00.0f, zRotation);
        float scale = getBoundScale(boundSize);
        if (scale == 0.0f) { scale = 0.0f; }
        floorOffset1 = (minZ - centerMassZ1) / scale;
    }
    ...
    private void readText(@NonNull InputStream stream) throws IOException {
        ...
        this.centerMassX1 = (float)(centerMassX / vertexCount);
        this.centerMassY1 = (float)(centerMassY / vertexCount);
        this.centerMassZ1 = (float)(centerMassZ / vertexCount);
